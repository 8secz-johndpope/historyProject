...
class AudioRecordManager: NSObject {
    ...
    /**
     获取录音权限并初始化录音
     */
    func checkPermissionAndSetupRecord() -> Bool {
        ...
        do {
            ...
            do {
                ...
                session.requestRecordPermission{allowed in
                    if !allowed {
                        TSAlertView_show(String.localize("LB_CA_IM_ACCESS_MICROPHONE_DENIED"), labelCancel: nil)
                    }
                    allow = allowed
                }
                if !allow {
                ...
                TSAlertView_show(String.localize("LB_CA_IM_ACCESS_MICROPHONE_PERMIT"), message: error.localizedFailureReason ?? "", labelCancel: nil)
                return false
            }
        } catch let error as NSError {
    ...
    func startRecord() {
        ...
        do {
            ...
            let recordSettings:[String : Any] = [
                //线性采样位数  8、16、24、32
                AVLinearPCMBitDepthKey: NSNumber(value: 8 as Int32),
            ...
            TSAlertView_show("初始化录音功能失败", message: error.localizedDescription, labelCancel: nil)
        }
    ...
    @objc func readyStartRecord() {
        ...
        do {
            ...
            TSAlertView_show("无法访问您的麦克风", message: error.localizedDescription, labelCancel: nil)
            return
        }
    ...
    func stopRecord() {
        if recorder != nil {
            DispatchQueue.main.async {
                self.delegate?.stopAudioRecording()
            }
            self.isFinishRecord = true
            self.isCancelRecord = false
            self.endTimer = CACurrentMediaTime()

            if (self.endTimer - self.startTime) < 1.0 {
                NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(AudioRecordManager.readyStartRecord), object: self)

                self.perform(#selector(AudioRecordManager.sharedInstance.cancelRecord), with: self, afterDelay: 0.4)

                dispatch_async_safely_to_main_queue({ () -> () in
                    self.delegate?.audioRecordTooShort()
                })
            } else {
                if (self.endTimer - self.startTime) >= 60 {
                    dispatch_async_safely_to_main_queue({ () -> () in
                        self.delegate?.audioRecordTooLong()
                    })
                }
                self.audioTimeInterval = NSNumber(value: NSNumber(value: recorder.currentTime as Double).floatValue as Float)
                if self.audioTimeInterval.floatValue < 1 {
                    self.perform(#selector(AudioRecordManager.sharedInstance.cancelRecord), with: self, afterDelay: 0.4)
                } else {
    ...
    func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully flag: Bool) {
        if flag && self.isFinishRecord {
            ...
            if TSVoiceConverter.convertWavToAmr(TempWavRecordPath.path, amrSavePath: TempAmrFilePath.path) {
                ...
                guard let amrAudioData = try? Data(contentsOf: TempWavRecordPath/* directly send wav in sprint 18*/) else {
                ...
                let recordTime: Float = AudioPlayManager.sharedInstance.getDurationFromPlayer(amrAudioData)
                self.delegate?.audioRecordFinish(amrAudioData, recordTime: recordTime, fileHash: fileName)
            } else {
                self.delegate?.audioRecordFailed()
            }
        } else {
