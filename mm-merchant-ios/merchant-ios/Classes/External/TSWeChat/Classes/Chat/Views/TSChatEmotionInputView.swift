...
class TSChatEmotionInputView: TSChatParentView {
    ...
    weak var delegate: ChatEmotionInputViewDelegate?
    ...
    override func awakeFromNib() {
        ...
        let itemWidth = (ScreenWidth - 10 * 2) / kNumberOfOneRow
        let padding = (ScreenWidth - kNumberOfOneRow * itemWidth) / 2.0
        let paddingLeft = padding
        let paddingRight = ScreenWidth - paddingLeft - itemWidth * kNumberOfOneRow
        ...
        self.groupDataSouce = self.chunk(self.emotionsDataSouce, size: kOneGroupCount)  //将数组切割成 每23个 一组
    ...
    //transpose line/row
    private func emoticonForIndexPath(_ indexPath: IndexPath) -> EmotionModel? {
