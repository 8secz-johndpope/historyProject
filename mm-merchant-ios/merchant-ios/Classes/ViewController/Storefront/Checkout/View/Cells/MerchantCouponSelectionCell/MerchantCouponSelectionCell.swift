//
//  MerchantCouponSelectionCell.swift
//  merchant-ios
//
...
class MerchantCouponSelectionCell: CouponSelectionCell {
    private let UnlimitedQuota = 999999999
    ...
    var data: Coupon? {
        didSet {
            if let coupon = data {
                pendingLabel.isHidden = !coupon.isPendingPayment()
                let couponInfo = NSMutableAttributedString()
                iconMerchant.round()
                
                if let merchantId = coupon.merchantId, let merchant = (merchantId == Constants.MMMerchantId ? Merchant.MM() : CacheManager.sharedManager.cachedMerchantById(merchantId)) {
                    ...
                    couponInfo.addAttributes([NSAttributedStringKey.font : UIFont.fontRegularWithSize(13),
                                              NSAttributedStringKey.foregroundColor : UIColor.secondary2()], range: NSRange(location: 0, length: merchantName.length))
                ...
                if let amount = coupon.couponAmount.formatPrice() {
                    ...
                    attString.addAttributes([
                        ...
                        NSAttributedStringKey.font : UIFont.fontRegularWithSize(14)
                        ],
                        range: NSRange(location: 0, length: 1))
                    attString.addAttributes([
                        NSAttributedStringKey.foregroundColor : UIColor.red,
                        NSAttributedStringKey.font : UIFont.fontRegularWithSize(24)
                        ],
                        range: NSRange(location: 1, length: fixedAmount.length - 1))
                ...
                btnCheckBox.isSelected = coupon.selected
                
                couponInfo.append(NSAttributedString(string: coupon.couponName + "\n"))
                couponInfo.addAttributes([NSAttributedStringKey.font : UIFont.fontRegularWithSize(12), NSAttributedStringKey.foregroundColor : UIColor.secondary2()], range: NSRange(location: couponInfo.length - coupon.couponName.length - 1, length: coupon.couponName.length))
                ...
                if couponDate != "" {
                    ...
                    couponInfo.addAttributes([NSAttributedStringKey.font : UIFont.fontRegularWithSize(11), NSAttributedStringKey.foregroundColor : UIColor.secondary3()], range: NSRange(location: couponInfo.length - couponDate.length - 1, length: couponDate.length))
                }
                
                if couponExpire != nil {
                    couponInfo.append(NSAttributedString(string: couponExpire!))
                    couponInfo.addAttributes([NSAttributedStringKey.font : UIFont.fontRegularWithSize(11), NSAttributedStringKey.foregroundColor : UIColor.red], range: NSRange(location: couponInfo.length - couponExpire!.length, length: couponExpire!.length))
                }
                
                if couponQuota != nil {
                    couponInfo.append(NSAttributedString(string: couponQuota!))
                    couponInfo.addAttributes([NSAttributedStringKey.font : UIFont.fontRegularWithSize(11), NSAttributedStringKey.foregroundColor : UIColor.red], range: NSRange(location: couponInfo.length - couponQuota!.length, length: couponQuota!.length))
                }
    ...
    override func awakeFromNib() {
        ...
        lblAmount.baselineAdjustment = UIBaselineAdjustment.alignCenters

        lblThreshold.textColor = .red
