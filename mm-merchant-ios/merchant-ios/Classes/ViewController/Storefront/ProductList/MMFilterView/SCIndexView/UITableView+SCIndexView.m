...
- (NSArray<NSNumber *> *)sc_selectedItemsDataSource {
    return objc_getAssociatedObject(self, @selector(sc_selectedItemsDataSource));
}

- (void)setSc_selectedItemsDataSource:(NSArray<NSNumber *> *)sc_selectedItemsDataSource {
    if (self.sc_selectedItemsDataSource == sc_selectedItemsDataSource) return;
    objc_setAssociatedObject(self, @selector(sc_selectedItemsDataSource), sc_selectedItemsDataSource, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (!sc_selectedItemsDataSource || sc_selectedItemsDataSource.count == 0) return;
    self.sc_indexView.selectedItemDataSource = sc_selectedItemsDataSource;
}
