...
- (instancetype)initWithTableView:(UITableView *)tableView configuration:(SCIndexViewConfiguration *)configuration
{
    if (self = [super initWithFrame:tableView.frame]) {
        ...
        if (configuration.indexViewStyle != SCIndexViewStyleNone) {
...
- (void)refreshTextLayer:(BOOL)selected
{
    ...
    if (self.selectedItemDataSource.count > 0) {
        for (NSNumber *index in _selectedItemDataSource) {
            NSInteger i = index.integerValue;
            CATextLayer *layer = self.subTextLayers[i];
            [CATransaction begin];
            [CATransaction setDisableActions:YES];
            layer.foregroundColor = self.configuration.IndexItemAllSelectTextColor.CGColor;
            [CATransaction commit];
        }
    }
}
...
- (void)setSelectedItemDataSource:(NSArray<NSNumber *> *)selectedItemDataSource {
    if (_selectedItemDataSource == selectedItemDataSource) return;
    _selectedItemDataSource = selectedItemDataSource.copy;
    for (CATextLayer *textLayer in self.subTextLayers) {
        [CATransaction begin];
        [CATransaction setDisableActions:YES];
        textLayer.foregroundColor = self.configuration.indexItemTextColor.CGColor;
        [CATransaction commit];
