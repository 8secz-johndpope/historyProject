...
class SegmentedControlView: UIView {
    ...
    private var SELECTOR_WIDTH_BUFFER: CGFloat = 0
    private var SEGMENT_HEIGHT: CGFloat = 45
    private var BOUNCE_BUFFER = 10
    private var ANIMATION_SPEED: CGFloat = 0.2
    private var SELECTOR_Y_BUFFER: CGFloat = 36 //default 36, adjust pos Y after frame init
    ...
    private var SELECTOR_WIDTH: CGFloat {
        get {
            ...
            if segmentedTitles.count > currentPageIndex {
                ...
                let currentWidth = StringHelper.getTextWidth(title, height: SEGMENT_HEIGHT, font: UIFont.systemFont(ofSize: segmentButtonFontSize))
                width = currentWidth
            }
    ...
    init(frame: CGRect, segmentedTitles: [String], hasRedDot: [Bool]? = nil, segmentButtonFontSize: CGFloat, selectedTitleColors: [UIColor], deSelectedTitleColor: UIColor, indicatorColors: [UIColor]) {
        ...
        self.segmentButtonFontSize = segmentButtonFontSize
        
        self.numOfPageCount = self.segmentedTitles.count
        
        super.init(frame: frame)
        SELECTOR_Y_BUFFER = frame.center.y + 14
    ...
    private func setup() {
        ...
        for i in 0 ..< numOfPageCount {
            ...
            let textWidth = StringHelper.getTextWidth(title, height: self.frame.size.height, font: UIFont.systemFont(ofSize: segmentButtonFontSize))
            let view = UIView(frame: CGRect(x: (button.frame.size.width + textWidth) / 2, y: 12, width: 5, height: 5))
            view.round()
            if let shouldShowRedDot = hasRedDot?[i] {
                view.isHidden = !shouldShowRedDot
            } else {
                view.isHidden = true
            }
            view.tag = reddotTag
            ...
            button.setTitleColor(deSelectedTitleColor, for: UIControlState())
    ...
    func updateIndicator() {
        ...
        let width = StringHelper.getTextWidth(title, height: SEGMENT_HEIGHT, font: UIFont.systemFont(ofSize: segmentButtonFontSize)) + SELECTOR_WIDTH_BUFFER
    ...
    open func updateReddot(hasReddot: [Bool]?) {
        guard let hasReddot = hasReddot else {
            return
        }
        
        for button in self.segmentButtons {
            let idx = button.tag
            for reddotView in button.subviews where reddotView.tag == reddotTag {
                reddotView.isHidden = !hasReddot[idx]
                break
            }
        }
    }
