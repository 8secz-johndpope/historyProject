...
public final class Navigator: NSObject {
    ...
    //specific open url for tb/pb
    open func sopen(_ url:String, params:QBundle? = nil, ext:Dictionary<String,NSObject>? = nil, modal:Bool? = nil, headView: UIView? = nil) {
        if let plp = self.getViewController(url, params: params) as? ProductListViewController, let headView = headView {
            headView.backgroundColor = .clear
            plp.headView = headView
            plp.navigationBarVisibility = .hidden
            
            let tc = topContainer()
            tc.add(controller: plp, at: -1)
        } else {
            self.dopen(url, params: params, ext: ext, modal: modal)
        }
    }
