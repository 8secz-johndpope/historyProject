...
class WishListSkuModel: DBModel {
...
//        self.Hashkey = keyword.ssn_md5()
    ...
    static func selectWishlistCartItemBy(brandId: Int) -> WishListSkuModel? {
        let tb = getWishTable()
        let results = tb.objects(WishListSkuModel.self, conditions: ["BrandId": brandId], sort:NSSortDescriptor(key: "LastModified", ascending: false))
    ...
    private static func getWishTable() -> DBTable {
        return DBUtil.table("wish_list_sku")
    }
